...
function toggleDrinkbar() {
  const drinkContainer = document.querySelector('.drink_bar'); // 對應 CSS 的 .drink_bar
  if (drinkContainer) {
    drinkContainer.classList.toggle('active');
  }
}

// 點擊頁面其他地方時，自動關閉跳出的飲料視窗
document.addEventListener('click', (e) => {
  const drinkIcon = document.getElementById('drinkMinIcon');
  const drinkContainer = document.querySelector('.drink_bar');
  
  if (drinkContainer && drinkIcon) {
    // 當點擊目標既不是 icon 本身，也不在飲料視窗內部時關閉
...
function toggleHandbag() {
  ...
  if (handbagContainer) {
    handbagContainer.classList.toggle('active');
  }
}
...
document.addEventListener('click', (e) => {
  ...
  if (handbagContainer && handbagIcon) {
